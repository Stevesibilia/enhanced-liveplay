# main-process-architecture Specification

## Purpose

Define the module structure of the Electron main process: single-concern modules under `electron/`, shared mutable state owned by `state.js`, explicit IPC handler registration, and unit-tested pure helpers in `electron/lib/`. Keeps `main.js` a thin bootstrap so features and agents touch only the module they change.
## Requirements
